(ns gyotaku.app
  "Gyotaku appview frontend shell.

  Migrated from the Svelte/Vite scaffold at
  appview/etzhayyim-wasm-gyotaku-i3zinrs2/svelte (a single `App.svelte` with a
  ...
  still a scaffold, not the archive UI described in the sibling
  kotodama.jsonld's `convoSystemPrompt` — see the repo README for what is and
  ...
  jp-go-dds's core components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-wasm-gyotaku-i3zinrs2\"
                         :description \"Gyotaku appview frontend shell (reagent + re-frame + jp-go-dds).\"
...
(def default-db
  ...
  {:page/heading "etzhayyim-wasm-gyotaku-i3zinrs2"
   :page/description "ClojureScript entry scaffold (reagent + re-frame + jp-go-dds) after the Svelte migration."})
